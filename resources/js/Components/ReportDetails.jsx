...
function Media({ path, alt }) {
    ...
    const fallback = typeof path === 'string' && !path.startsWith('http')
        ? `/storage/${path.replace(/^\/+/, '').replace(/^storage\//, '').replace(/^public\//, '')}`
        : null;
    const imageSrc = failed && fallback && fallback !== src ? fallback : src;

    if (!imageSrc || (failed && imageSrc === src)) {
        return <div style={{ minHeight: 120, borderRadius: 14, background: '#F8FAFC', border: '1px dashed #CBD5E1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8' }}><i className="ti ti-photo-off" style={{ fontSize: 32 }} /></div>;
    }

    return <img src={imageSrc} alt={alt || ''} onError={() => setFailed(true)} style={{ width: '100%', maxHeight: 340, objectFit: 'cover', display: 'block', borderRadius: 14 }} />;
}
...
function UserProfileCard({ user, onClose }) {
    ...
    const statusLabel = user?.status === 'active' ? 'نشط' : user?.status === 'inactive' ? 'موقوف' : (user?.status || 'غير محدد');
    const verifyLabel = verification?.status === 'approved' ? 'موثق' : verification?.status === 'rejected' ? 'مرفوض' : verification ? 'قيد المراجعة' : 'غير موثق';
    const field = (label, value) => <div style={{ minWidth: 0 }}><div style={{ fontSize: 10, color: '#94A3B8', marginBottom: 3 }}>{label}</div><div style={{ fontSize: 12.5, fontWeight: 700, color: '#0F172A', wordBreak: 'break-word' }}>{value || 'غير متوفر'}</div></div>;

    return <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(15,23,42,.66)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <div onClick={event => event.stopPropagation()} dir="rtl" style={{ width: '100%', maxWidth: 1040, maxHeight: '94vh', overflowY: 'auto', background: '#F8FAFC', borderRadius: 22, boxShadow: '0 28px 90px rgba(15,23,42,.35)' }}>
            <div style={{ position: 'sticky', top: 0, zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '16px 22px', background: 'rgba(255,255,255,.96)', borderBottom: '1px solid #E2E8F0', backdropFilter: 'blur(8px)' }}>
                <div style={{ fontSize: 18, fontWeight: 900, color: '#1E1B4B' }}>الملف الكامل للمستخدم</div>
                <button type="button" onClick={onClose} aria-label="إغلاق" style={{ width: 34, height: 34, border: '1px solid #E2E8F0', borderRadius: 9, background: '#fff', color: '#64748B', cursor: 'pointer', fontSize: 18 }}><i className="ti ti-x" /></button>
            </div>
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <section style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 18, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                    <Avatar user={user} size={86} />
                    <div style={{ flex: 1, minWidth: 220 }}><div style={{ fontSize: 22, fontWeight: 900, color: '#0F172A' }}>{fullName || 'مستخدم بدون اسم'}</div><div style={{ color: '#64748B', fontSize: 12, marginTop: 4 }}>رقم المستخدم: #{user?.id}</div><div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 10 }}><span className="report-detail-chip">الحساب: {statusLabel}</span><span className="report-detail-chip">التوثيق: {verifyLabel}</span><span className="report-detail-chip">المنشورات: {user?.posts?.length ?? 0}</span><span className="report-detail-chip">الخدمات: {services.length}</span></div></div>
                </section>
                <section style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 18 }}><div style={{ fontSize: 15, fontWeight: 900, color: '#1E1B4B', marginBottom: 14 }}>البيانات الشخصية والتواصل</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14 }}>{field('الاسم الأول', user?.first_name)}{field('الاسم الأوسط', user?.middle_name)}{field('الكنية', user?.last_name)}{field('البريد الإلكتروني', user?.email)}{field('رقم الهاتف', user?.phone)}{field('تاريخ الميلاد', user?.birthdate)}{field('الجنس', user?.gender)}{field('المدينة', user?.city)}{field('خط العرض', user?.latitude)}{field('خط الطول', user?.longitude)}{field('تاريخ التسجيل', user?.created_at ? new Date(user.created_at).toLocaleString('ar') : null)}</div></section>
                <section style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 18 }}><div style={{ fontSize: 15, fontWeight: 900, color: '#1E1B4B', marginBottom: 14 }}>التحقق من الهوية</div>{verification ? <><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14, marginBottom: 16 }}>{field('الحالة', verifyLabel)}{field('الاسم في الهوية', verification.full_name)}{field('رقم الهوية', verification.id_number)}{field('نوع الهوية', verification.id_type)}{field('نسبة التطابق', verification.match_score != null ? `${verification.match_score}%` : null)}{field('سبب الرفض', verification.rejection_reason)}</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 12 }}>{[['صورة الهوية الأمامية', verification.front_image], ['صورة الهوية الخلفية', verification.back_image], ['صورة السيلفي', verification.selfie_image]].map(([label, path]) => <div key={label}><div style={{ fontSize: 11, fontWeight: 800, color: '#64748B', marginBottom: 7 }}>{label}</div><Media path={path} alt={label} /></div>)}</div></> : <div style={{ padding: 16, borderRadius: 10, background: '#FFFBEB', color: '#92400E', fontSize: 13 }}>لم يرفع المستخدم طلب توثيق أو صور هوية.</div>}</section>
                <section style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 18 }}><div style={{ fontSize: 15, fontWeight: 900, color: '#1E1B4B', marginBottom: 14 }}>حساب الأعمال الكامل</div>{business ? <><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14, marginBottom: 14 }}>{field('اسم النشاط', business.name)}{field('المهنة', business.name_job)}{field('رقم التواصل', business.number)}{field('الحالة', business.status)}{field('المدينة', business.city)}{field('المنطقة', business.area)}{field('الشارع', business.street)}{field('نوع النشاط', business.activity)}{field('خط العرض', business.latitude)}{field('خط الطول', business.longitude)}</div><div style={{ padding: 12, background: '#F8FAFC', borderRadius: 10, color: '#475569', fontSize: 12, lineHeight: 1.8 }}>{business.description || 'لا يوجد وصف للنشاط.'}</div>{business.image && <div style={{ marginTop: 14 }}><div style={{ fontSize: 11, fontWeight: 800, color: '#64748B', marginBottom: 7 }}>صورة النشاط</div><Media path={business.image} alt="صورة النشاط" /></div>}{business.gallery?.length > 0 && <div style={{ marginTop: 14 }}><div style={{ fontSize: 11, fontWeight: 800, color: '#64748B', marginBottom: 7 }}>معرض الأعمال ({business.gallery.length})</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 10 }}>{business.gallery.map(image => <Media key={image.id} path={image.image} alt={image.caption} />)}</div></div>}</> : <div style={{ padding: 16, borderRadius: 10, background: '#F8FAFC', color: '#64748B', fontSize: 13 }}>لا يوجد حساب أعمال لهذا المستخدم.</div>}</section>
                <section style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 16, padding: 18 }}><div style={{ fontSize: 15, fontWeight: 900, color: '#1E1B4B', marginBottom: 14 }}>الخدمات ({services.length})</div>{services.length ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 12 }}>{services.map(service => <div key={service.id} style={{ border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden', background: '#fff' }}>{service.image && <Media path={service.image} alt={service.name} />}<div style={{ padding: 13 }}><div style={{ fontSize: 14, fontWeight: 900, color: '#0F172A' }}>{service.name || 'خدمة بدون اسم'}</div><div style={{ color: '#475569', fontSize: 12, lineHeight: 1.7, marginTop: 5 }}>{service.description || 'لا يوجد وصف.'}</div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 9 }}><span className="report-detail-chip">{service.category?.name || 'بدون فئة'}</span><span className="report-detail-chip">{service.subcategory?.name || 'بدون تصنيف فرعي'}</span><span className="report-detail-chip">{service.city?.name || 'بدون مدينة'}</span><span className="report-detail-chip">{service.status || 'بدون حالة'}</span><span className="report-detail-chip">{Number(service.price || 0).toLocaleString()} {service.price_type === 'usd' ? 'USD' : 'SYP'}</span></div></div></div>)}</div> : <div style={{ padding: 16, borderRadius: 10, background: '#F8FAFC', color: '#64748B', fontSize: 13 }}>لا توجد خدمات مسجلة لهذا المستخدم.</div>}</section>
            </div>
        </div>
    </div>;
}

export default function ReportDetails({ report, target, targetType, userProfile, warnings = [], Layout, backHref, actionPrefix }) {
    ...
    return <Layout title="تفاصيل البلاغ">
        ...
        {profileOpen && userProfile && <UserProfileCard user={userProfile} onClose={() => setProfileOpen(false)} />}
    </Layout>;
}
