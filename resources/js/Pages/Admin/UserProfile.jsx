    ...
    return (
        <AdminLayout title={fullName}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: C.textFaint }}>
                <Link href="/admin/users" style={{ color: T, textDecoration: 'none' }}>المستخدمون</Link>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
                        <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                                <span style={{ fontSize: 21, fontWeight: 800, color: C.textDark }}>{fullName}</span>
                ...
                <SectionCard icon="building-store" title="معلومات النشاط التجاري" subtitle={business.name ?? ''}>
                    <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12 }}>
                            {business.number && (
                                <div>
                                    <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 2 }}>رقم التواصل</div>
                                    <div style={{ fontSize: 13, fontWeight: 600, color: C.textDark }}>{business.number}</div>
                                </div>
                            )}
                            <div>
                                <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 2 }}>حالة الحساب التجاري</div>
                        ...
                        {business.description && (
                            <div style={{ background: '#F8FAFC', borderRadius: 10, padding: '12px 16px' }}>
                                <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 6 }}>نبذة عن النشاط</div>
                        ...
                        {profile.services.map(s => (
                            <div key={s.id} style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, borderRight: '1px solid #F1F5F9' }}>
                                ...
                                <div style={{ minWidth: 0 }}>
                                    <div style={{ fontSize: 13, fontWeight: 600, color: C.textDark, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.name}</div>
                                    <div style={{ fontSize: 11, color: C.textFaint, marginTop: 2 }}>{s.category?.name ?? ''}{s.city?.name ? ` · ${s.city.name}` : ''}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        {profile.posts.map((post, i) => (
                            <div key={post.id} style={{ padding: '14px 22px', borderTop: i === 0 ? 'none' : '1px solid #F8FAFC' }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: C.textDark, marginBottom: 4 }}>{post.title}</div>
            ...
            {!business && !profile.services?.length && !profile.posts?.length && (
                <div style={{ textAlign: 'center', padding: '48px 24px', color: C.textFaint, background: '#fff', borderRadius: 18, border: C.cardBorder }}>
