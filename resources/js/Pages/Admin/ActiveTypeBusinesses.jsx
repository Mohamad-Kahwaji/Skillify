...
export default function ActiveTypeBusinesses({ types }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post('/admin/active-type-businesses', { onSuccess: () => { reset(); setShowCreate(false); } });
    };
    ...
    return (
        <AdminLayout title="أنواع الأعمال">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: '#0F172A' }}>أنواع الأعمال</div>
            ...
            {showCreate && (
                <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 12, padding: 18 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>نوع عمل جديد</div>
                    ...
                    <tbody>
                        {!(types ?? []).length ? (
                            <tr><td colSpan={4} style={{ padding: '40px', textAlign: 'center', color: '#94A3B8' }}>
                                <i className="ti ti-briefcase" style={{ fontSize: 32, display: 'block', marginBottom: 8, opacity: 0.3 }} />
                                لا توجد أنواع أعمال بعد
                            </td></tr>
                        ) : (types ?? []).map((t, i) => (
