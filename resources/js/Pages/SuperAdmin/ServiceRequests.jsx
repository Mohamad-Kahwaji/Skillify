...
export default function ServiceRequests({ services = [] }) {
    ...
    const approve = (id) => router.patch(`/super-admin/service-requests/${id}/approve`, {}, { preserveScroll: true });
    const reject  = (id) => router.patch(`/super-admin/service-requests/${id}/reject`,  {}, { preserveScroll: true });
    const pending = (id) => router.patch(`/super-admin/service-requests/${id}/pending`, {}, { preserveScroll: true });

    return (
        <SuperAdminLayout title="طلبات الخدمات">
            <Head title="طلبات الخدمات — Skillify" />

            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div>
                    <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1E1B4B', margin: 0, letterSpacing: -0.5 }}>طلبات الخدمات</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 12 }}>
                {[
                    { label: 'إجمالي الطلبات', val: counts.all,      icon: 'ti-file-text',    c: '#7C3AED', bg: '#F5F3FF' },
                    ...
                    {filtered.map((s, i) => {
                        ...
                        return (
                            <div key={s.id} style={{
                                ...
                                borderRight: s.status === 'pending' ? '3px solid #0D9488' : '3px solid transparent',
                                ...
                                <div style={{ flex: 1, minWidth: 200 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                            ...
                                            <span style={{ fontSize: 12, color: '#475569', fontWeight: 600 }}>{s.user?.first_name} {s.user?.last_name}</span>
                                        </div>
                                    ...
                                    {s.description && <p style={{ fontSize: 12, color: '#64748B', lineHeight: 1.55, marginTop: 6, marginBottom: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{s.description}</p>}
                                </div>

                                <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexShrink: 0 }}>
                                    {s.status !== 'approved' && (
                                        <button onClick={() => approve(s.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 9, cursor: 'pointer', border: '1px solid #6EE7B7', background: '#ECFDF5', color: '#065F46', fontSize: 12, fontWeight: 700, fontFamily: "'Cairo','Inter',sans-serif" }}>
                                            <i className="ti ti-check" /> قبول
                                        </button>
                                    )}
                                    {s.status !== 'rejected' && (
                                        <button onClick={() => reject(s.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 9, cursor: 'pointer', border: '1px solid #FCA5A5', background: '#FEF2F2', color: '#991B1B', fontSize: 12, fontWeight: 700, fontFamily: "'Cairo','Inter',sans-serif" }}>
                                            <i className="ti ti-x" /> رفض
                                        </button>
                                    )}
                                    {s.status !== 'pending' && (
                                        <button onClick={() => pending(s.id)} title="إعادة للمراجعة" style={{ width: 32, height: 32, borderRadius: 8, cursor: 'pointer', border: '1px solid #E2E8F0', background: '#F8FAFC', color: '#64748B', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
