...
export default function ActiveTypeBusinesses({ types }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post('/super-admin/active-type-businesses', {
    ...
    const saveEdit = (id) => {
        ...
        setSaving(true);
        router.patch(`/super-admin/active-type-businesses/${id}`, { name: editName.trim() }, {
            preserveScroll: true,
    ...
    const destroy = (id) => {
        ...
        router.delete(`/super-admin/active-type-businesses/${id}`, { preserveScroll: true });
    };

    const all = types ?? [];

    return (
        <SuperAdminLayout title="أنواع الأعمال">
            ...
            {showCreate && (
                <div style={{ background: '#fff', border: '1px solid rgba(124,58,237,0.15)', borderRadius: 16, padding: '22px 24px', boxShadow: '0 4px 20px rgba(124,58,237,0.08)' }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#1E1B4B', marginBottom: 16 }}>نوع عمل جديد</div>
                    <form onSubmit={submit}>
                        <div style={{ marginBottom: 16 }}>
                            <label style={{ fontSize: 11, fontWeight: 700, color: '#6B7280', display: 'block', marginBottom: 6 }}>اسم النوع *</label>
                            <input dir="rtl" autoFocus style={{ ...INPUT, maxWidth: 320 }} placeholder="مثال: مهنة" value={data.name} onChange={e => setData('name', e.target.value)} required />
            ...
            <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.06)', borderRadius: 18, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.05)' }}>
                ...
                {!all.length ? (
                    <div style={{ padding: '56px', textAlign: 'center', color: '#94A3B8' }}>
                        <i className="ti ti-briefcase" style={{ fontSize: 40, display: 'block', marginBottom: 10, opacity: 0.15 }} />
                        <div style={{ fontSize: 13 }}>لا توجد أنواع أعمال</div>
                    </div>
                ) : all.map((t, i) => (
                    <div key={t.id}
                        ...
                        {editingId === t.id ? (
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <span style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg,#EDE9FE,#DDD6FE)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <i className="ti ti-briefcase" style={{ color: P, fontSize: 14 }} />
                                </span>
