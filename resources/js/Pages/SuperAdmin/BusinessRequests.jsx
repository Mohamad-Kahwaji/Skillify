...
const TABS = [
    { key: 'all',      label: 'الكل',          icon: 'ti-apps',         color: '#3730A3', bg: '#EEF2FF', border: '#C7D2FE' },
...
export default function BusinessRequests({ businesses = [] }) {
    ...
    const approve = (id) => router.patch(`/super-admin/business-requests/${id}/approve`, {}, { preserveScroll: true });
    const reject  = (id) => router.patch(`/super-admin/business-requests/${id}/reject`,  {}, { preserveScroll: true });
    const pending = (id) => router.patch(`/super-admin/business-requests/${id}/pending`, {}, { preserveScroll: true });

    return (
        <SuperAdminLayout title="طلبات حسابات الأعمال">
            <Head title="طلبات حسابات الأعمال — Skillify" />

            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                <div>
                    <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1E1B4B', margin: 0, letterSpacing: -0.5 }}>طلبات حسابات الأعمال</h1>
                ...
                <div style={{ position: 'relative' }}>
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)}
                        ...
                        style={{ width: 250, padding: '9px 38px 9px 14px', border: '1px solid rgba(0,0,0,0.09)', borderRadius: 10, fontSize: 13, outline: 'none', boxSizing: 'border-box', fontFamily: "'Cairo','Inter',sans-serif", background: '#FAFAFA', direction: 'rtl' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 12 }}>
                {[
                    { label: 'إجمالي الطلبات', val: counts.all,      icon: 'ti-briefcase',    c: '#7C3AED', bg: '#F5F3FF' },
                    ...
                    <div key={s.label} style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 12, padding: '14px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                            <div style={{ fontSize: 20, fontWeight: 800, color: '#1E1B4B', lineHeight: 1 }}>{s.val}</div>
            ...
            {/* Cards */}
            {!filtered.length ? (
                <div style={{ padding: '72px 24px', textAlign: 'center', background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.05)' }}>
                    <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#F5F3FF', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                        <i className="ti ti-briefcase" style={{ fontSize: 28, color: '#C4B5FD' }} />
                    </div>
                    ...
                    {filtered.map((b, i) => {
                        ...
                        return (
                            <div key={b.id} style={{
                                ...
                                borderRight: b.status === 'pending' ? '3px solid #0D9488' : '3px solid transparent',
                            ...
                            >
                                ...
                                <div style={{ flex: 1, minWidth: 200 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 5 }}>
                                        ...
                                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, padding: '2px 9px', borderRadius: 20, background: st.bg, color: st.color, border: `1px solid ${st.border}` }}>
                                            <span style={{ width: 5, height: 5, borderRadius: '50%', background: st.dot }} />
                                            {st.label}
                                        </span>
                                    </div>

                                    {b.name_job && <div style={{ fontSize: 12.5, color: '#0D9488', fontWeight: 600, marginBottom: 6 }}>{b.name_job}</div>}
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                            ...
                                            <span style={{ fontSize: 12, color: '#475569', fontWeight: 600 }}>{b.user?.first_name} {b.user?.last_name}</span>
                                            {b.user?.email && <span style={{ fontSize: 11, color: '#94A3B8' }}>· {b.user.email}</span>}
                                        </div>
                                        {b.city && <span style={{ fontSize: 11, color: '#94A3B8', display: 'flex', alignItems: 'center', gap: 3 }}><i className="ti ti-map-pin" style={{ fontSize: 11 }} />{b.city}</span>}
                                        <span style={{ fontSize: 11, color: '#94A3B8', display: 'flex', alignItems: 'center', gap: 3 }}><i className="ti ti-clock" style={{ fontSize: 11 }} />{timeAgo(b.created_at)}</span>
                                    </div>

                                    {b.description && <p style={{ fontSize: 12, color: '#64748B', lineHeight: 1.55, marginTop: 7, marginBottom: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{b.description}</p>}
                                </div>

                                <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
                                    {b.status !== 'active' && (
                                        <button onClick={() => approve(b.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '8px 16px', borderRadius: 10, cursor: 'pointer', border: 'none', background: 'linear-gradient(135deg,#10B981,#059669)', color: '#fff', fontSize: 12.5, fontWeight: 700, fontFamily: "'Cairo','Inter',sans-serif", boxShadow: '0 3px 10px rgba(16,185,129,0.3)' }}>
                                            <i className="ti ti-check" /> قبول
                                        </button>
                                    )}
                                    {b.status !== 'rejected' && (
                                        <button onClick={() => reject(b.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '8px 14px', borderRadius: 10, cursor: 'pointer', border: '1px solid #FCA5A5', background: '#FEF2F2', color: '#991B1B', fontSize: 12.5, fontWeight: 700, fontFamily: "'Cairo','Inter',sans-serif" }}>
                                            <i className="ti ti-x" /> رفض
                                        </button>
                                    )}
                                    {b.status !== 'pending' && (
                                        <button onClick={() => pending(b.id)} title="إعادة للمراجعة" style={{ width: 34, height: 34, borderRadius: 9, cursor: 'pointer', border: '1px solid #E2E8F0', background: '#F8FAFC', color: '#64748B', fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
