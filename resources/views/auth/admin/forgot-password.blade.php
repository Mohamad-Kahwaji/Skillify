...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .brand-icon{width:48px;height:48px;border-radius:14px;background:var(--accent);display:inline-flex;align-items:center;justify-content:center;color:#fff;font-size:22px;margin-bottom:12px;}
    .brand-name{font-size:22px;font-weight:600;letter-spacing:-.4px;}
    .brand-sub{font-size:13px;color:var(--text-muted);margin-top:2px;}
    .card{background:var(--bg-surface);border:0.5px solid var(--border);border-radius:14px;padding:28px;}
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="card-sub">Enter your admin email to receive a reset link</div>
    ...
    <form method="POST" action="{{ route('admin.forgot-password.send') }}">
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          ...
          <input type="email" name="email" value="{{ old('email') }}" placeholder="admin@hirfa.com" autofocus required>
        </div>
        @error('email')<div class="field-error">{{ $message }}</div>@enderror
      </div>
      <button type="submit" class="btn-submit">Send Reset Link</button>
    </form>
    <div class="back-link"><a href="{{ route('admin.login') }}">← Back to Login</a></div>
  </div>
</div>
</body>
</html>
