  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar {
      ...
      background: var(--surface); border-left: 0.5px solid var(--border); height: 100vh;
    }
    .sidebar-head {
    ...
    .icon-btn {
      ...
      color: var(--text-sec); font-size: 18px; transition: background 0.12s;
    }
    .icon-btn:hover { background: var(--sunken); }
    .search-wrap { padding: 10px 12px; border-bottom: 0.5px solid var(--border); flex-shrink: 0; }
    .search-inner {
    ...
    .search-inner input {
      ...
      padding: 8px 4px; font-size: 13px; color: var(--text); font-family: var(--font);
    }
    ...
    .other-avatar {
      ...
      display: flex; align-items: center; justify-content: center;
      font-size: 14px; font-weight: 600; color: #fff; flex-shrink: 0; position: relative;
    }
    .online-dot {
    ...
    .msg-sender-name { font-size: 11px; color: var(--text-muted); margin-bottom: 2px; padding: 0 4px; }
    .msg-row.sent .msg-sender-name { text-align: right; }
    .bubble {
      padding: 10px 14px; border-radius: 18px; font-size: 13px;
      line-height: 1.5; max-width: 100%; word-break: break-word; position: relative;
    }
    .msg-row.sent .bubble {
      background: var(--bubble-sent); color: var(--msg-sent-text); border-bottom-right-radius: 4px;
    }
    .msg-row.received .bubble {
      background: var(--bubble-recv); color: var(--msg-recv-text);
      border-bottom-left-radius: 4px; border: 0.5px solid var(--border);
    ...
    .msg-row.sent .file-bubble { background: rgba(255,255,255,.15); border: 0.5px solid rgba(255,255,255,.2); }
    .msg-row.received .file-bubble { background: var(--sunken); border: 0.5px solid var(--border); }
    .file-icon-wrap {
    ...
    .img-bubble { border-radius: 12px; overflow: hidden; max-width: 260px; cursor: pointer; }
    .img-bubble img { width: 100%; display: block; transition: opacity 0.15s; }
    .img-bubble:hover img { opacity: 0.9; }
    .video-bubble { border-radius: 12px; overflow: hidden; max-width: 280px; }
    .video-bubble video { width: 100%; display: block; }
    .msg-time {
      font-size: 10px; color: var(--text-muted); padding: 0 4px; margin-top: 2px;
    ...
    /* ── INPUT AREA ─────────────────────────────── */
    .chat-input-wrap { background: var(--surface); border-top: 0.5px solid var(--border); padding: 12px 16px; flex-shrink: 0; }
    .file-preview-bar {
      display: none; align-items: center; gap: 10px;
      padding: 8px 12px; margin-bottom: 8px;
      background: var(--sunken); border: 0.5px solid var(--border-md); border-radius: 10px;
    }
    .file-preview-bar.visible { display: flex; }
    .fp-thumb { width: 40px; height: 40px; border-radius: 8px; object-fit: cover; border: 0.5px solid var(--border); }
    .fp-icon {
    ...
    .fp-clear { background: none; border: none; font-size: 18px; color: var(--text-muted); padding: 2px; border-radius: 4px; transition: color 0.12s; }
    ...
    .msg-input-wrap {
      ...
      background: var(--sunken); border: 0.5px solid var(--border-md);
      border-radius: 20px; padding: 0 14px; gap: 8px; transition: border-color 0.15s;
    }
    ...
    .send-btn {
      width: 38px; height: 38px; flex-shrink: 0;
      display: flex; align-items: center; justify-content: center;
      border-radius: 50%; border: none; background: var(--accent); color: #fff; font-size: 18px;
            ...
            <div class="msg-content">
              @if($msg->file_type === 'image' && $msg->file_path)
                <div class="img-bubble" onclick="openLightbox('{{ Storage::url($msg->file_path) }}')">
                  <img src="{{ Storage::url($msg->file_path) }}" alt="{{ $msg->file_name }}" loading="lazy" />
                </div>
...
<script>
  ...
  function clearFile() {
    ...
    fpMedia.innerHTML = ''; fpName.textContent = '';
  }
  ...
  async function sendMessage(e) {
    ...
    const optimisticEl = buildMessageEl({
      id: tempId, user_id: MY_ID, message_text: text || null,
    ...
    scrollToBottom(true);
    msgInput.value = ''; msgInput.style.height = 'auto'; clearFile();
    try {
      const resp = await fetch(SEND_URL, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF }, body: formData });
      ...
      const el = document.querySelector(`[data-id="${tempId}"]`);
      if (el) { el.style.opacity = '0.5'; el.title = 'Failed to send'; }
    } finally {
  ...
  function buildMessageEl(msg, isMine) {
    ...
    let avatarHtml = '';
    if (!isMine) avatarHtml = `<div class="msg-avatar" style="background:${OTHER_COLOR};">${OTHER_INITIAL}</div>`;
    let bodyHtml = '';
    if (msg.file_type === 'image' && (msg.file_url || msg.file_path)) {
      const url = msg.file_url || (STORAGE_BASE + msg.file_path);
      bodyHtml = `<div class="img-bubble" onclick="openLightbox('${url}')"><img src="${url}" alt="${msg.file_name || ''}" loading="lazy" /></div>`;
    } else if (msg.file_type === 'video' && (msg.file_url || msg.file_path)) {
      const url = msg.file_url || (STORAGE_BASE + msg.file_path);
      bodyHtml = `<div class="video-bubble bubble" style="padding:0;"><video controls src="${url}"></video></div>`;
      ...
      bodyHtml = `<div class="bubble" style="padding:0;"><a href="${url}" target="_blank" style="text-decoration:none;color:inherit;" download="${msg.file_name || ''}"><div class="file-bubble"><div class="file-icon-wrap"><i class="ti ${icon}"></i></div><div class="file-info"><div class="file-name">${msg.file_name || 'File'}</div><div class="file-dl">Click to download</div></div><i class="ti ti-download" style="font-size:16px;opacity:.6;"></i></div></a></div>`;
    } else if (msg.message_text) {
      const escaped = msg.message_text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
    ...
    row.innerHTML = `${avatarHtml}<div class="msg-content">${bodyHtml}<div class="msg-time">${timeStr} ${checksHtml}</div></div>`;
    return row;
  }
  ...
  async function markConversationRead() {
    try {
      await fetch(MARK_READ_URL, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' } });
  ...
  function showChatToast({ senderName, preview, conversationId, senderId }) {
    ...
    setTimeout(() => { toast.style.transition='opacity .25s, transform .25s';toast.style.opacity='0';toast.style.transform='translateX(-12px)';setTimeout(()=>toast.remove(),250); }, 5000);
  }
  ...
  if (window.Echo) {
    window.Echo.private(`conversation.${CONVERSATION_ID}`)
      .listen('.message.sent', (data) => {
        ...
        const el = buildMessageEl({ id: data.message_id, user_id: data.user_id, message_text: data.message_text, file_type: data.file_type, file_name: data.file_name, file_path: data.file_path, created_at: data.send_date || new Date().toISOString() }, false);
        appendMessage(el); scrollToBottom(true); markConversationRead();
        const convItem = document.querySelector(`.conv-item[href*="/chat/${CONVERSATION_ID}"]`);
        if (convItem) { const lastEl = convItem.querySelector('.conv-last'); if (lastEl) lastEl.textContent = data.message_text || '📎 File'; }
      });
    window.Echo.private(`notifications.${MY_ID}`)
      .listen('.new.message', (data) => {
        if (!data || data.conversation_id == CONVERSATION_ID) return;
        showChatToast({ senderName: data.sender_name || 'User', preview: data.message_text ? data.message_text.substring(0,60) : (data.file_name ? '📎 '+data.file_name : 'New message'), conversationId: data.conversation_id, senderId: data.user_id || 0 });
        const convItem = document.querySelector(`.conv-item[href*="/chat/${data.conversation_id}"]`);
        if (convItem) { const lastEl = convItem.querySelector('.conv-last'); if (lastEl) lastEl.textContent = data.message_text || '📎 File'; const timeEl = convItem.querySelector('.conv-time'); if (timeEl) timeEl.textContent = 'Now'; document.getElementById('conv-list').prepend(convItem); }
      });
  }
