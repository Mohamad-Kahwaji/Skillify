...
@section('breadcrumb', 'Activity Types')

@section('content')
<div class="page-head">
  <div>
    <div class="page-title">Activity Types</div>
    <div class="page-sub">{{ $types->count() }} type(s) registered</div>
  </div>
</div>
...
<div class="content-grid" style="grid-template-columns:1fr 340px;">
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <form method="POST" action="{{ route('admin.active_types.destroy', $type->id) }}"
  ...
  <div class="card" style="padding:24px;">
    <div class="card-title" style="margin-bottom:20px;">Add Activity Type</div>
    @if(session('success'))
      <div class="alert success"><i class="ti ti-check"></i> {{ session('success') }}</div>
    @endif
    <form method="POST" action="{{ route('admin.active_types.store') }}">
      @csrf
      <div style="margin-bottom:14px;">
        ...
        <div style="display:flex;align-items:center;background:var(--bg-sunken);border:0.5px solid var(--border-md);border-radius:8px;overflow:hidden;">
          <i class="ti ti-tag" style="padding:0 11px;font-size:16px;color:var(--text-muted);"></i>
